import {
  ...
  export class ApurementController {
    ...
    @Get('sa/:saId')
    async getAllocationsForSa(@Param('saId') saId: string) {
      return this.apurementService.getAllocationsForSa(saId);
    }
  
    @Get('ea/:eaId')
    async getAllocationsForEa(@Param('eaId') eaId: string) {
      return this.apurementService.getAllocationsForEa(eaId);
    }
    @Get('sa/:saId')
